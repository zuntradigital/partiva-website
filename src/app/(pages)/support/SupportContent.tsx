...
export default function SupportContent() {
  ...
  // Overrides just the WhatsApp option's number/link with the DB-backed
  // value once fetched; the static defaults above (already matching what's
  // published today) serve as the initial state and the fallback on failure.
  const [whatsapp, setWhatsapp] = useState<{ linkLabel: string; href: string } | null>(null);
  useEffect(() => {
    fetch(`${API}/api/contact`, { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : null))
      .then((r) => {
        if (r?.success && r.data.whatsappNumber && r.data.whatsappLink) {
          setWhatsapp({ linkLabel: r.data.whatsappNumber, href: r.data.whatsappLink });
        }
      })
      .catch(() => {});
  }, []);
  const options = content.options.map((option) => (option.id === "whatsapp" && whatsapp ? { ...option, ...whatsapp } : option));
  ...
  useEffect(() => {
    fetch(`${API}/api/pages`, { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : null))
      .then((r) => { if (r?.success) setSections(resolveMainAndExtras(r.data, "support")); })
      .catch(() => {});
  }, []);
      ...
      <section className="mt-14" aria-labelledby="support-options-heading">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {options.map((option, i) => {
            ...
            return (
              <RevealOnScroll key={option.id} className="h-full" amount={0.35} margin="0px 0px -10%" delay={(i % 3) * 0.08}>
              <article
                className="flex h-full flex-col rounded-2xl border border-neutral-200 bg-white p-6 text-center shadow-sm transition-shadow hover:shadow-md dark:border-neutral-800 dark:bg-neutral-900"
              >
