...
export default function TrustedBySection({
  ...
  return (
    <section dir={isArabic ? "rtl" : "ltr"} className="bg-neutral-50 py-14 dark:bg-[#060c20]" lang={locale} data-language-managed>
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-6">
          {brands.map((brand, index) => (
            <div key={brand} className={`motion-enter motion-delay-${Math.min(index + 1, 5)} flex items-center gap-2 text-gray-400 dark:text-slate-500`}>
              <ShieldCheck className="h-5 w-5 text-blue-600 dark:text-blue-400" strokeWidth={1.75} />
