...
export default function ArticlesList({
  ...
  const copy = locale === "ar"
    ? { badge: "مركز المعرفة", title: override?.bodyAr ? (override.titleAr || "مقالات تساعدكِ على إدارة أعمالك بذكاء") : "مقالات تساعدكِ على إدارة أعمالك بذكاء", description: override?.bodyAr || "أدلة عملية ونصائح بسيطة لتنظيم المخزون والمبيعات والعملاء في مكان واحد.", categories: "تصنيفات المقالات", list: "قائمة المقالات", empty: "لا توجد مقالات في هذا التصنيف حاليًا.", previous: "السابق", next: "التالي", pagination: "ترقيم صفحات المقالات" }
    : { badge: "Knowledge Center", title: override?.bodyEn ? (override.titleEn || "Practical insights to manage your business smarter") : "Practical insights to manage your business smarter", description: override?.bodyEn || "Clear guides for organizing inventory, sales, and customers in one place.", categories: "Article categories", list: "Article list", empty: "There are no articles in this category yet.", previous: "Previous", next: "Next", pagination: "Article pagination" };
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3" aria-label={copy.list}>
          {paginatedArticles.map((article, index) => (
            <RevealOnScroll key={article.slug} variant="up" delay={(index % 6) * 0.06}>
